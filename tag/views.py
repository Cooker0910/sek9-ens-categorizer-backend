...
class TagDetail(APIView):
  ...
  @swagger_auto_schema(
    request_body=TagSerializer(many=False),
    responses={200: TagSerializer(many=False)}
  )
...
class TagCreate(APIView):
  ...
  @swagger_auto_schema(
      request_body=NewTagSerializer(many=False),
      responses={200: TagSerializer(many=False)}
  )
  def post(self, request, format=None):
    serializer = NewTagSerializer(data=request.data, many=False)
    if serializer.is_valid():
      # Create new member with serializer
      new_item = Tag.objects.create(**serializer.validated_data)
      new_serializer = TagSerializer(new_item, many=False)
